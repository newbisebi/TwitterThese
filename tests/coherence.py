...
def test_all_text_processed():
    ...
    """
    nb_empty_text_field = (
        session.query(TL).filter(TL.texte_retraite == "").count())
...
def test_rt_completed():
    ...
    nb_empty_rt_field = (
        session.query(TL).filter(TL.nb_rt == None).count())  # noqa
...
def test_fav_completed():
    ...
    nb_empty_fav_field = (
        session.query(TL).filter(TL.nb_favori == None).count())  # noqa
...
def test_date_influence_not_empty():
    ...
    nb_empty_dinfluence_field = (
        session.query(TL).filter(TL.date_influence == None).count())  # noqa
...
def test_envir3_not_empty():
    ...
    nb_empty_envir3_field = (
        session.query(TL).filter(TL.envir3 == None).count())  # noqa
